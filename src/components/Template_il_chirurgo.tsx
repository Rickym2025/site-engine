...
import { Phone, Calendar, Star, Check, Send, Sparkles, Mail, MapPin, ChevronDown, Shield, ArrowRight } from 'lucide-react';
...
export default function Template_il_chirurgo({ data, nomeCliente }: TemplateProps) {
  ...
  const fallbackEmail = email || `servizi@${nomeCliente.toLowerCase().replace(/[^a-z0-9]/g, '')}.it`;
  const fallbackIndirizzo = indirizzo || "Viale dell'Innovazione 104, Centro Direzionale (VE)";
  ...
  return (
    ...
    >
      ...
      <header className="border-b border-zinc-900 bg-black/40 backdrop-blur-md sticky top-0 z-50">
        <div className="max-w-6xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center space-x-8 text-sm font-semibold text-zinc-400">
            <a href="#contatto" className="hover:text-white transition-colors">Richiesta</a>
            <a href="#servizi" className="hover:text-white transition-colors">Vantaggi</a>
      ...
      <section className="max-w-6xl mx-auto px-6 pt-16 pb-24 grid grid-cols-1 lg:grid-cols-12 gap-12 items-center relative z-10">
        ...
        <div className="lg:col-span-6 space-y-6 text-left">
          <motion.div
            ...
            className="inline-flex items-center space-x-2 bg-zinc-900/60 border border-[color:var(--brand-color-strong)] px-4 py-1.5 rounded-full"
          >
            <Sparkles className="h-4 w-4 text-[color:var(--brand-color)] animate-pulse" />
            <span className="text-xs font-mono text-zinc-350 uppercase tracking-wider font-semibold">Zero Frizioni • Contatto Diretto</span>
          </motion.div>

          <h1 className="text-4xl md:text-6xl font-black tracking-tight leading-[1.1] bg-gradient-to-b from-white to-zinc-400 bg-clip-text text-transparent">
            {hero.headline}
          </h1>
          ...
          <div className="space-y-4 pt-4">
            {servizi.map((servizio: any, idx: number) => (
              <div key={idx} className="flex items-start space-x-3">
                <div className="bg-[color:var(--brand-color-glow)] p-1 rounded-full mt-1 border border-[color:var(--brand-color-strong)]">
                  <Check className="h-4 w-4 text-[color:var(--brand-color)] font-bold" />
                </div>
                <div>
                  <h4 className="font-bold text-zinc-200 text-lg">{servizio.titolo}</h4>
                  <p className="text-sm text-zinc-450 leading-relaxed font-light mt-1">{servizio.descrizione}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Destra: Modulo di Contatto sopra la piega */}
        <motion.div 
          id="contatto"
          initial={{ opacity: 0, y: 30 }}
          animate={{ opacity: 1, y: 0 }}
          className="lg:col-span-6 bg-zinc-950/80 border border-zinc-900 p-8 rounded-3xl shadow-2xl backdrop-blur-md relative"
        >
          {inviato ? (
            ...
            <form onSubmit={handleFormSubmit} className="space-y-5">
              <div className="text-center mb-6">
                <h3 className="text-2xl font-bold mb-1">Invia la tua Richiesta</h3>
                <p className="text-xs text-zinc-500 font-mono">Consulenza professionale immediata</p>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full bg-white/[0.01] border border-zinc-855 focus:border-[color:var(--brand-color)] rounded-xl p-4 text-white placeholder-zinc-650 transition-all outline-none text-sm"
                />
              </div>
              ...
              <div>
                ...
                <select className="w-full bg-zinc-900 border border-zinc-855 focus:border-[color:var(--brand-color)] rounded-xl p-4 text-white transition-all outline-none text-sm appearance-none cursor-pointer">
      ...
      {/* TRUST PROOF BANNER */}
      <section id="recensioni" className="bg-zinc-950 border-y border-zinc-900 py-12 px-6 text-center relative z-10">
      ...
      {/* DETTAGLIO SERVIZI */}
      <section id="servizi" className="py-20 px-6 max-w-6xl mx-auto relative z-10 border-t border-zinc-900/60">
